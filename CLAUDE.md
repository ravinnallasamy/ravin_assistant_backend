...
Groq (`groq-sdk`, same `GROQ_API_KEY`) is used for both `speechToText` (Whisper transcription) and answer generation — a single Groq client/key now covers the whole LLM surface of this app. Gemini has been fully removed (`@google/generative-ai` uninstalled, `GEMINI_API_KEY` no longer read anywhere).
